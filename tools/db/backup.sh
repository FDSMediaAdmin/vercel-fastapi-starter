...
IFS=$'\n' read -d '' -r -a table_names <<< $(docker run --rm -e MYSQL_PWD="$db_password" mysql:8.0 \
  mysql -h host.docker.internal -P 3306 -u "$db_user" -N -B -e "USE $db_name; SHOW TABLES;")
...
for table_name in $table_names; do
  ...
  docker run --rm -e MYSQL_PWD="$db_password" -v "$db_backup_dir:/backups" mysql:8.0 \
    sh -c "exec mysqldump -h host.docker.internal -P 3306 -u$db_user $db_name $table_name" > "$table_backup_file"
