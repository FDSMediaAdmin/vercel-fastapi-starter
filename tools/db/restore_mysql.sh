...
db_exists() {
  ...
  if docker run --rm -e MYSQL_PWD="$db_password" "$image" \
...
create_database() {
  ...
  if [ "$db_exists_result" -eq 0 ] && [ "$can_continue_result" -eq 0 ]; then
    docker run --rm -e MYSQL_PWD="$db_password" "$image" \
...
import_sql_files2() {
  ...
  if [ -n "$subfolder" ]; then
    docker run --rm -v "$backup_dir/$subfolder:/backup" -e MYSQL_PWD="$db_password" "$image" \
  ...
  if [ -n "$subfolder" ]; then
    ...
    if [ ${#sql_files[@]} -eq 0 ]; then
      ...
      for sql_file in "${sql_files[@]}"; do
        ...
        cat "$sql_file" | docker run --rm -i -e MYSQL_PWD="$db_password" "$image" \
